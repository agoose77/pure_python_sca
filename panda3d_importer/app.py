...
class PandaLogicImporter(SCALogicImporter):
    ...
    def _build_event_manager(self, data):
        ...
        self.sensor_builders.update({'KEYBOARD': self.build_keyboard, 'MOUSE': self.build_mouse})

        return super(PandaLogicImporter, self)._build_event_manager(data)

    def build_mouse(self, sensor_data):
        name = sensor_data['name']

        sensor = MouseSensor(name)
        sensor.mouse_event = sensor.get_panda_event_name(sensor_data['mouse_event'])
    ...
    def build_keyboard(self, sensor_data):
        ...
        sensor.key = sensor.get_panda_event_name(sensor_data['key'])
        sensor.modifier_1 = sensor.get_panda_event_name(sensor_data['modifier_key_1'])
        sensor.modifier_2 = sensor.get_panda_event_name(sensor_data['modifier_key_2'])
    ...
    def build_motion(self, actuator_data):
        ...
        actuator.mode = actuator_data['mode']
        actuator.delta_location = actuator_data['offset_location']
        actuator.delta_rotation = actuator_data['offset_rotation']
